...
> (~51s, 1280×800, with an original synthesized backing track —
> `store/video/backing-track.m4a`, composed in the repo's tooling, so licensing
> is unambiguously ours; `record-video.mjs` muxes it on and warns loudly if the
> file is missing rather than silently shipping a mute cut, which is exactly
> what happened between Jul 31 and Aug 18) and `store/video/shelf-demo.gif` (the build-it beats),
