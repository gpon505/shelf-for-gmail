...
- **Add a note:** hover a row → click the **note icon** → type → Esc. The note appears as a compact colored chip right after the subject line in every list view (including the inbox); hover the chip to see the full note, click to edit. The editor supports **bold/italic/underline** (buttons or ⌘B/⌘I/⌘U), five colors (yellow/red/green/blue/gray — e.g. red for urgent), a ✕ to delete the note, and **⌘/Ctrl+Enter to save & close**. Picking a non-yellow color also paints a slim matching edge on the thread's row, so urgency reads at a glance even where the chip is out of view. Inside an open conversation, a note icon sits in the toolbar (next to ⋮), and the note shows as a sticky-note strip under the subject — click it and it becomes editable **in place**, no popup.
