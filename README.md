...
- **Known limitation — `j`/`k` navigates in date order, not shelf order.** In a label with shelves, Gmail's own cursor still walks the list in the order Gmail stores it, so pressing `j` can land somewhere unexpected on screen. Shelf's shortcuts always act on the right thread; it's the *travel* between threads that reads oddly. This is not a bug we can quietly fix — see the keyboard-cursor note in [ARCHITECTURE.md](ARCHITECTURE.md). A Shelf-owned cursor that moves in the order you see is in **Labs** (`labs.cursor`, Alt+J / Alt+K) while it earns trust; it adds keys rather than taking Gmail's, so Gmail's own navigation is untouched.
